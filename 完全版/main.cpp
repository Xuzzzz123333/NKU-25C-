...
// GameMode枚举已在game_mode.h中定义

bool is_debug = false;  // 调试模式
GameMode current_game_mode = GameMode::NONE;  // 当前游戏模式
...
bool prevent_scene_switch = false;  // 场景切换保护标志
int scene_switch_delay = 0;         // 场景切换延迟计时器

// 添加场景切换延迟处理函数
void process_scene_switch_delay(int delta) {
    if (prevent_scene_switch && scene_switch_delay > 0) {
        scene_switch_delay -= delta;
        if (scene_switch_delay <= 0) {
...
{
	AddFontResourceEx(_T("resources/Flx.ttf"), FR_PRIVATE, NULL);
	...
	mciSendString(_T("open resources/pea_break_1.mp3 aliaspea_break_1"), NULL, 0, NULL);
	mciSendString(_T("open resources/pea_break_2.mp3 aliaspea_break_2"), NULL,0 , NULL);
	mciSendString(_T("open resources/pea_break_3.mp3 aliaspea_break_3"), NULL, 0, NULL);
	...
	mciSendString(_T("open resources/fall.mp3 alias fall"), NULL, 0, NULL);
	mciSendString(_T("open resources/ji.mp3 alias ji"), NULL, 0, NULL);
}
int main()
{
	ExMessage msg;
	...
	while (1)
	{
		...
		// 处理场景切换延迟
		process_scene_switch_delay(delta_tick);
	...
	// 婵炴挸鎳愰幃濠勬導閸曨剛鐖?	delete menu_scene;
	delete game_scene;
